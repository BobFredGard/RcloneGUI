...
function showTab(tabId) {
    ...
    if (tabId === 'overview') renderOverview(window.currentBackups || []);
}
...
// Vue d'ensemble : tableau compact en lecture seule (toutes les planifiées d'un coup d'oeil)
function isOverviewRunning(b) {
    const now = Date.now();
    if (b.pid !== null && b.pid !== undefined) return true;
    if (b.started_at) {
        const d = new Date(String(b.started_at).replace(' ', 'T') + 'Z');
        if (!isNaN(d.getTime()) && (now - d.getTime()) < 120000) return true;
    }
    if (b.last_run) {
        const d = new Date(String(b.last_run).replace(' ', 'T') + 'Z');
        if (!isNaN(d.getTime()) && (now - d.getTime()) < 120000) return true;
    }
    return false;
}

function formatOverviewLastRun(lastRun) {
    if (!lastRun) return 'Jamais';
    try {
        const d = new Date(String(lastRun).replace(' ', 'T'));
        if (isNaN(d.getTime())) return 'Jamais';
        return d.toLocaleString('fr-FR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' });
    } catch { return 'Jamais'; }
}

function renderOverview(backups) {
    const body = document.getElementById('overview-body');
    const summary = document.getElementById('overview-summary');
    if (!body) return;
    const list = [...(backups || [])].sort((a, b) => String(a.name).localeCompare(String(b.name), 'fr'));

    let running = 0, failed = 0, scheduled = 0;
    body.innerHTML = list.length ? list.map(b => {
        const isRunning = isOverviewRunning(b);
        if (isRunning) running++;
        if (b.last_status === 'failed') failed++;
        if (b.schedule_type === 'on') scheduled++;

        let badge;
        if (isRunning) badge = '<span class="ov-badge running">⚡ En cours</span>';
        else if (b.schedule_type === 'off') badge = '<span class="ov-badge off">⏸ OFF</span>';
        else if (b.last_status === 'success') badge = '<span class="ov-badge success">✓ OK</span>';
        else if (b.last_status === 'failed') badge = '<span class="ov-badge failed">✗ Échec</span>';
        else if (b.last_status === 'cancelled') badge = '<span class="ov-badge cancelled">✗ Annulé</span>';
        else badge = '<span class="ov-badge waiting">⏳ En attente</span>';

        const sched = b.schedule_type === 'on' ? ('ON' + (b.night_only ? ' 🌙' : '')) : 'OFF';
        const dest = b.destination_path || b.dropbox_path || '';
        const tags = [b.bidirectional ? ' ⇄' : '', b.night_only ? ' 🌙' : ''].join('');
        return `<tr>
            <td class="ov-name">${escapeHtml(b.name)}${tags}</td>
            <td>${badge}</td>
            <td>${formatOverviewLastRun(b.last_run)}</td>
            <td>${escapeHtml(sched)}</td>
            <td class="ov-path">${escapeHtml(b.source_path || '')}</td>
            <td class="ov-path">${destIcon(b.destination_type || 'dropbox')} ${escapeHtml(dest)}</td>
        </tr>`;
    }).join('') : '<tr><td colspan="6" style="text-align:center;color:var(--text-secondary);">Aucune sauvegarde.</td></tr>';

    if (summary) summary.textContent = `${scheduled} programmée(s) • ${running} en cours • ${failed} en échec`;
}
...
function connectProgressSSE(id) {
    ...
    const es = new EventSource(`/api/backups/${id}/stream?token=${encodeURIComponent(currentToken || '')}`);
    eventSources[id] = es;
